...
function hasSavedSession() {
  try {
    ...
    // Pairing attempts can set `registered` or `me.id` before WhatsApp has
    // delivered the account payload. A reusable session always has both the
    // account data and an identity, so do not mistake a partial attempt for a
    // completed login.
  ...
  Module._load = function loadSaffulPairingBaileys(request, parent, isMain) {
    ...
    const createSaffulSocket = (options = {}) => {
      const number = global.__saffulAuthMethod === 'pairing'
        ...
        : '';
      const linked = Boolean(options.auth?.creds?.account && options.auth?.creds?.me?.id && options.auth?.creds?.advSecretKey);
      const pairingOptions = number && !linked
        ? {
          ...options,
          ...(global.__saffulPairingVersion ? { version: global.__saffulPairingVersion } : {}),
          // Match MEGA-MD's registered desktop companion profile.
          browser: loaded.Browsers?.macOS?.('Chrome') || ['Mac OS', 'Chrome', ''],
          defaultQueryTimeoutMs: 60_000,
          markOnlineOnConnect: true,
        }
        : options;
      const socket = previousCreateSocket(pairingOptions);
      ...
      socket.ev.on('connection.update', (update = {}) => {
        ...
        if (connection === 'close' && global.__saffulPairingRequested) {
          ...
          authLog(`[auth] Pairing connection closed: ${reason}`);
        }
      });
