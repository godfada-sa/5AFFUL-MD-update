...
// Expose trim for the memory watchdog in index.js
// The store instance registers itself here after creation so index.js can call trim.
if (typeof global !== 'undefined') {
  const origTrim = global.__saffulStoreTrim;
  global.__saffulStoreTrim = function () {
    // We can't directly access the store from here, but the store's trim()
    // method is exposed via this closure. We set it once the store is created.
    if (typeof origTrim === 'function') origTrim();
  };
}
