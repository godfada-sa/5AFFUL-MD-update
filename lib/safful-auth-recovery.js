...
function hasRegisteredSession() {
  try {
    ...
    // Lenient check: during WhatsApp session rotations (status 515), some
    // credential fields can be temporarily absent while the session is being
    // refreshed. Only consider the session truly gone if creds.json is missing
    // or has no `me.id` at all. The `account` and `advSecretKey` fields may
    // be temporarily null during rotation — don't archive the session for that.
...
// A process supervisor (PM2, Pterodactyl, systemd) restarts the bot itself
// the moment it exits. Spawning a detached child here AS WELL created two
// concurrent instances — one booting fresh/pairing while another already owns
// the session. They fight over the same folder, the pairing codes get refused
// (the account is already linked by the other instance), and the log shows
// duplicate QRs and doubled boot sequences. Under a supervisor, exit cleanly
// and let it restart exactly one instance. The detached restart is only for
// bare `node index.js` runs with no supervisor.
function underSupervisor() {
  return Boolean(
    process.env.PM2_HOME
    || process.env.NODE_APP_INSTANCE !== undefined
    || process.env.P_SERVER_UUID
    || process.cwd() === '/home/container',
  );
}
...
function restartFreshLogin(exit) {
  ...
  archiveFailedAuth();
  if (underSupervisor()) {
    console.warn('[auth] Supervisor detected — exiting so it restarts a single instance (no duplicate spawn).');
    return exit(0);
  }
...
function restartSavedSession(exit) {
  if (underSupervisor()) {
    console.log('[auth] Pairing completed — exiting so the supervisor restarts with the newly saved session (no duplicate spawn).');
    return exit(0);
  }
  ...
  const child = spawn(process.execPath, [path.join(PROJECT_ROOT, 'index.js')], {
    ...
    // standard startup now preserves those credentials as well.
    env: { ...process.env },
    windowsHide: false,
  });
  child.once('error', (error) => {
