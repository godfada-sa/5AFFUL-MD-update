...
const SPAM_LIMIT = 5;
const VIEW_ONCE_CACHE_DIR = path.join(__dirname, '..', 'temp', 'antidelete-cache');
...
// WhatsApp can deliver a message to the bot as a placeholder stub instead of
// its real content (e.g. it arrived undecryptable and the phone never supplied
// the bytes via placeholder-resend). Such messages carry a stub type but no
// message content. Resolve the enum once instead of hardcoding the number.
const CIPHERTEXT_STUB_TYPE = Number(proto?.WebMessageInfo?.StubType?.CIPHERTEXT);
const STUB_TYPE_BY_NAME = {};
try {
  for (const [name, value] of Object.entries(proto.WebMessageInfo.StubType || {})) {
    if (typeof value === 'number') STUB_TYPE_BY_NAME[value] = name;
  }
} catch {}
...
function isRevoke(update = {}) {
  // Baileys 7 emits revocations as messages.update with `message: null` and
  // messageStubType set to StubType.REVOKE (1). Older Baileys versions
  // embedded the protocolMessage instead. Both arrive through the same event.
  const protocol = update.message?.protocolMessage;
  if (protocol) return protocol.type === 0 || protocol.type === 'REVOKE';
  return update.messageStubType === 'REVOKE' || update.messageStubType === 1;
}
...
async function streamToBuffer(stream, maxBytes = Infinity) {
  const chunks = [];
  let total = 0;
  const STREAM_TIMEOUT_MS = 60 * 1000;  // 60s per-chunk timeout
...
// A message truly carries readable content when its protobuf `message` field
// holds at least one content type (imageMessage, conversation, ...). Empty
// envelopes (message: null / {}) are what WhatsApp sends when it withholds
// view-once content from this session, or when a LID/phone-number remap
// delivers only a stub copy of an otherwise content-bearing message id.
function hasMessageContent(message = {}) {
  const content = message?.message;
  return Boolean(content && typeof content === 'object' && !Array.isArray(content) && Object.keys(content).length > 0);
}

// WhatsApp 2026 addresses contacts by both their phone number and their LID.
// The same message id can be delivered under either jid, and a revoke may
// reference the other one.
function alternateJid(jid) {
  if (typeof jid !== 'string') return null;
  if (jid.endsWith('@lid')) return `${jid.slice(0, -4)}@s.whatsapp.net`;
  if (jid.endsWith('@s.whatsapp.net')) return `${jid.slice(0, -15)}@lid`;
  return null;
}

function cacheEarlyRawMessage(message) {
  const antiDeleteOn = antiDeleteEnabled();
  ...
  // The bot stores every view-once it receives, whether or not anti-delete is
  // already on: the media URL stops working the moment the message is
  // revoked, and the operator may toggle anti-delete on afterwards. Only
  // view-once bytes are captured unconditionally — ordinary media is still
  // cached only while anti-delete is on, so there is no background bandwidth
  // for messages that can simply be forwarded from the original.
  if (media?.isViewOnce) {
  ...
  if (!antiDeleteOn && !media?.isViewOnce) return;

  const hasContent = hasMessageContent(rawMessage);
  const cacheKey = `${key.remoteJid}:${key.id}`;
  const previous = earlyCachedMessages.get(cacheKey);
  earlyCachedMessages.set(cacheKey, cached);
  // Recovery looks up by message id when the revoke's chat jid differs from
  // the original (LID vs phone number). The id map must never let an empty
  // envelope evict a content-bearing copy of the same id.
  const existingById = earlyCachedMessagesById.get(key.id);
  if (!existingById || hasContent || !hasMessageContent(existingById.message)) {
    earlyCachedMessagesById.set(key.id, cached);
  }
  // Index under the alternate addressing mode too, so a revoke arriving with
  // the other jid still finds this entry.
  const alt = alternateJid(key.remoteJid);
  if (alt) earlyCachedMessages.set(`${alt}:${key.id}`, cached);
...
function getEarlyCachedMessage(remoteJid, id) {
  if (!id) return null;
  const primary = earlyCachedMessages.get(`${remoteJid}:${id}`);
  if (primary) return primary;
  const alt = alternateJid(remoteJid);
  const viaAlt = alt ? earlyCachedMessages.get(`${alt}:${id}`) : null;
  if (viaAlt) return viaAlt;
  return earlyCachedMessagesById.get(id) || null;
}
...
// Install before the legacy core is loaded. The core adds and can mutate its
// own message listener while it creates the socket, so attaching only after
// `bot.connect()` is too late for a real view-once envelope.
function installEarlyCaptureHook() {
  if (global.__saffulAntiDeleteEarlyCaptureInstalled) return;
  global.__saffulAntiDeleteEarlyCaptureInstalled = true;

  const corePath = path.join(__dirname, 'smd.js');
  const previousLoad = Module._load;
  Module._load = function loadSaffulPreCoreMediaCapture(request, parent, isMain) {
    const loaded = previousLoad.call(this, request, parent, isMain);
    if (request !== '@whiskeysockets/baileys' || parent?.filename !== corePath || typeof loaded?.default !== 'function') {
      return loaded;
    }

    const createSocket = loaded.default;
    const createSocketWithEarlyMediaCapture = (options = {}) => {
      const sock = createSocket(options);
      installSocketRawCapture(sock, true);
      return sock;
    };
    ...
    return new Proxy(loaded, {
      get(target, property, receiver) {
        return property === 'default'
          ? createSocketWithEarlyMediaCapture
...
// envelope.
function installSocketRawCapture(sock, isPreCore = false) {
  ...
  process.stdout.write(isPreCore
    ? '[anti-delete] pre-core raw media capture attached\n'
    : '[anti-delete] raw message boundary capture attached\n');
  return true;
}
...
// Relay the deleted message to the recovery chat as an ordinary message. The
// original media URL and keys survive in the pre-revoke copy, so no download
// or re-upload is required — the view-once restriction is simply stripped.
// This mirrors how the legacy core's copyNForward(…, { readViewOnce: true })
// works, but handles every wrapper modern WhatsApp uses (viewOnceMessage,
// viewOnceMessageV2, viewOnceMessageV2Extension, deviceSentMessage, …).
async function reforwardDeletedMessage(sock, recipient, message) {
  let raw = message?.message;
  if (!raw || typeof raw !== 'object') return false;
  if (raw.deviceSentMessage?.message) raw = raw.deviceSentMessage.message;
  ...
  // WhatsApp refuses to relay messages still flagged viewOnce. The media URL
  // and decryption keys remain valid, so clearing the flag is enough. Set it
  // explicitly instead of deleting: protobufjs turns deleted optional fields
  // into null, which is fine on the wire but obscures the intent here.
  if (typeof inner === 'object' && ['imageMessage', 'videoMessage', 'audioMessage', 'documentMessage'].includes(type)) {
    inner.viewOnce = false;
  }

  const waMessage = await generateWAMessageFromContent(recipient, { [type]: inner }, {});
  await sock.relayMessage(recipient, waMessage.message, { messageId: waMessage.key.id });
  return true;
}
...
// A WhatsApp-generated stub (CIPHERTEXT placeholder, deleted-for-me notice,
// group event, ...) has no real content. Reporting it as a "decode failure" is
// misleading: the bot never received the bytes, so nothing could be decoded.
function stubLabel(message = {}) {
  const stubType = message?.messageStubType;
  if (!stubType) return null;
  const stubName = typeof stubType === 'number' ? (STUB_TYPE_BY_NAME[stubType] || `stub-${stubType}`) : String(stubType);
  if (stubType === CIPHERTEXT_STUB_TYPE || stubName === 'CIPHERTEXT') {
    return '[Delivered as a placeholder — WhatsApp did not provide the content to the bot]';
  }
  return `[System message${stubName && stubName !== `stub-${stubType}` ? ` (${stubName})` : ''}]`;
}

// Last resort when a message truly carries no readable content and no stub
// type: describe what WhatsApp actually sent instead of a generic failure.
// This is self-diagnosing — if a future wrapper is missed, the report shows
// the envelope shape so the wrapper list can be extended.
function describeUndecodable(message = {}) {
  const msg = message || {};
  const messageField = 'message' in msg ? msg.message : undefined;
  const messageType = messageField === null
    ? 'an empty envelope (message: null)'
    : `a ${Array.isArray(messageField) ? `array(${messageField.length})` : typeof messageField} envelope`;
  const contentKeys = messageField && typeof messageField === 'object' && !Array.isArray(messageField)
    ? Object.keys(messageField).join(', ')
    : '';
  const topKeys = Object.keys(msg).join(', ') || 'none';
  return `[No readable content — WhatsApp sent ${messageType}${contentKeys ? ` containing: ${contentKeys}` : ''} (fields: ${topKeys})]`;
}
...
function describeMessage(message) {
  ...
  const content = unwrapContent(message.message || message);
  if (!content || typeof content !== 'object') return stubLabel(message) || describeUndecodable(message);
  ...
  if (content.contactMessage) {
    const display = content.contactMessage.displayName
      || String(content.contactMessage.vcard || '').match(/FN:([^\r\n]*)/i)?.[1]?.trim()
      || '';
    return display ? `[Contact: ${display}]` : '[Contact]';
  }
  if (content.locationMessage) {
    const { degreesLatitude: lat, degreesLongitude: lng } = content.locationMessage;
    if (lat !== undefined && lng !== undefined) return `[Location: ${lat}, ${lng}]`;
    return '[Location]';
  }
  if (content.pollMessage?.name) return `[Poll: ${content.pollMessage.name}]`;
  if (content.reactionMessage) return '[Reaction]';
  if (content.buttonsMessage?.title) return `[Buttons: ${content.buttonsMessage.title}]`;
  if (content.listMessage?.title) return `[List: ${content.listMessage.title}]`;
  if (content.templateMessage) {
    const hydrated = content.templateMessage?.hydratedTemplate?.title || content.templateMessage?.hydratedFourRowTemplate?.title;
    if (typeof hydrated === 'string') return `[Message: ${hydrated}]`;
    return '[Message]';
  }
  return stubLabel(message) || describeUndecodable(message);
}
...
// Group names are resolved lazily and cached briefly so the recovery card can
// name the group without hammering WhatsApp metadata on every deletion.
...
function withTimeout(promise, ms) {
  return Promise.race([
    ...
    new Promise((_, reject) => {
      setTimeout(() => reject(new Error('group metadata timed out')), ms);
    }),
  ]);
}

async function resolveGroupName(sock, chatId) {
  if (!chatId || !/@(g\.us|lid)$/.test(chatId)) return '';
  const cached = groupNameCache.get(chatId);
  if (cached && Date.now() - cached.at < GROUP_NAME_TTL_MS) return cached.name;

  let name = '';
  // Store-backed names need no network round-trip, but only trust them for
  // @g.us chats; a private @lid address must never be shown as a "group".
  if (chatId.endsWith('@g.us') && typeof sock?.getName === 'function') {
    try {
      const resolved = shortText(await sock.getName(chatId));
      if (resolved && !resolved.includes('@') && !/unknown/i.test(resolved)) name = resolved;
    } catch {}
  }
  if (!name && typeof sock?.groupMetadata === 'function') {
    try {
      const meta = await withTimeout(sock.groupMetadata(chatId), 6000);
      const subject = shortText(meta?.subject);
      if (subject) name = subject;
    } catch {}
  }
  groupNameCache.set(chatId, { name, at: Date.now() });
  if (groupNameCache.size > 200) {
    const oldestKey = groupNameCache.keys().next().value;
    groupNameCache.delete(oldestKey);
  }
  return name;
}

function recoveryCard(name, deletedMessage, group) {
  const preview = shortText(deletedMessage) || '[Deleted message content could not be decoded]';
  const indentedPreview = preview.split('\n').map((line) => `│ ${line || ' '}`).join('\n');
  const lines = ['┌─〔 *SAFFUL ANTI-DELETE* 〕'];
...
module.exports = function attachProtection(sock) {
  ...
  // WhatsApp can emit the same revocation more than once — once per
  // addressing mode (LID vs phone number) or once as messages.update and
  // again as an upsert protocolMessage. Each delivery would otherwise re-send
  // the recovered media and report card to sudo.
  const recentlyRecoveredIds = new Map();
  ...
  function remember(message) {
    ...
    // Same content-preference rule as the early cache: an empty envelope must
    // not evict a content-bearing copy of the same id.
    const existingById = cachedMessagesById.get(key.id);
    if (!existingById || hasMessageContent(cached.message) || !hasMessageContent(existingById.message)) {
  ...
  async function recover(targetKey) {
    ...
    const candidateEntries = [
      getEarlyCachedMessage(targetKey.remoteJid, targetKey.id),
      cachedMessages.get(`${targetKey.remoteJid}:${targetKey.id}`),
      cachedMessagesById.get(targetKey.id),
    ].filter(Boolean);
    // Prefer the copy that actually carries content: the same message id can
    // exist twice (LID vs phone-number addressing) where one copy is an empty
    // envelope. Never settle for the empty one when a content-bearing copy
    // was cached.
    const cachedEntry = candidateEntries.find((entry) => hasMessageContent(entry.message)) || candidateEntries[0];
    const cached = cachedEntry?.message;
      ...
      // Only claim the id once we actually have something to recover, so a
      // duplicate revocation that arrives after content is cached can still
      // retry the not-yet-cached case.
      const now = Date.now();
      const previousRecovery = recentlyRecoveredIds.get(targetKey.id);
      if (previousRecovery && now - previousRecovery < RECOVERY_DEDUP_MS) {
        console.log('[anti-delete] duplicate revocation ignored (already recovered)', targetKey.id);
        return;
      }
      recentlyRecoveredIds.set(targetKey.id, now);
      if (recentlyRecoveredIds.size > 100) {
        for (const [id, recoveredAt] of recentlyRecoveredIds) {
          if (now - recoveredAt > RECOVERY_DEDUP_MS) recentlyRecoveredIds.delete(id);
        }
      }
      const recipient = sudoJid();
      if (!recipient) return;
      const senderName = await contactName(sock, cached);
      const chatLabel = await resolveGroupName(sock, targetKey.remoteJid);
      const deletedMessage = describeMessage(cached);
      // Content-less / unsupported messages (view-once withheld by WhatsApp,
      // placeholder stubs, empty envelopes) have nothing to recover — the bot
      // never received the bytes. Log the diagnostic to the console only and
      // stay SILENT to sudo: a "[No readable content …]" card is noise, not a
      // recovery.
      const unsupportedContent = deletedMessage.includes('could not be decoded')
        || deletedMessage.includes('No readable content')
        || deletedMessage.includes('placeholder');
      if (unsupportedContent) {
        // process.stdout.write so the diagnostic survives the legacy core's
        // console.log silencing on panels without MSGS_IN_LOG set.
        const source = getEarlyCachedMessage(targetKey.remoteJid, targetKey.id) === cachedEntry ? 'early-cache' : 'later-cache';
        const messageField = cached?.message;
        const contentKeys = messageField && typeof messageField === 'object' && !Array.isArray(messageField)
          ? Object.keys(messageField).join(',')
          : '';
        process.stdout.write(
          `[anti-delete] DIAG content-less message id=${targetKey.id} chat=${targetKey.remoteJid} source=${source} `
          + `stubType=${cached?.messageStubType ?? 'none'} stubParams=${JSON.stringify(cached?.messageStubParameters || [])} `
          + `messageField=${messageField === null ? 'null' : typeof messageField} contentKeys=[${contentKeys}] `
          + `topKeys=[${Object.keys(cached || {}).join(',')}] — NOT reported to sudo (unsupported content)\n`
        );
      ...
      const report = recoveryCard(senderName, deletedMessage, chatLabel);

      try {
        // Forwarding a view-once envelope is intentionally rejected by
        // WhatsApp. Recovery order:
        //   1. Bytes captured while the original was fresh (most reliable).
        //   2. Re-forward the original media with its URL + keys, with the
        //      view-once restriction stripped (no download or re-upload).
        //   3. Last resort: download the original URL and re-upload it.
        let recoveredMedia = false;
        const capturedViewOnce = cachedEntry?.viewOnceCapture ? await cachedEntry.viewOnceCapture : null;
        if (capturedViewOnce) {
          try {
            recoveredMedia = await recoverViewOnceMedia(sock, recipient, cached, capturedViewOnce);
            if (recoveredMedia) process.stdout.write('[anti-delete] re-sent recovered view-once media to sudo\n');
          } catch (error) {
            console.warn('[anti-delete] view-once media resend from cache failed:', error.message || error);
          }
        }
        if (!recoveredMedia) {
          try {
            recoveredMedia = await reforwardDeletedMessage(sock, recipient, cached);
            if (recoveredMedia) process.stdout.write('[anti-delete] re-forwarded recovered media to sudo\n');
          } catch (error) {
            console.warn('[anti-delete] media re-forward failed:', error.message || error);
          }
        }
        if (!recoveredMedia) {
          try {
            recoveredMedia = await recoverViewOnceMedia(sock, recipient, cached, null);
          } catch (error) {
            console.warn('[anti-delete] view-once media download/resend failed:', error.message || error);
          }
        }
...
module.exports.isRevoke = isRevoke;
module.exports.reforwardDeletedMessage = reforwardDeletedMessage;
