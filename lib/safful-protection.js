...
// The legacy core subscribes to the same Baileys event and normalizes some
// message envelopes. Keep an untouched copy here, before that listener can
// alter a view-once V2 envelope or discard its media metadata.
...
function cacheEarlyRawMessage(message) {
  ...
  if (key.fromMe || isOwner(senderFromKey(key))) return;
...
// This is installed before lib/smd.js is required. It decorates just the
// socket constructed by that legacy core, attaching our listener first. The
// command handler still receives its normal events; anti-delete merely gets a
// faithful read-only copy before it transforms the message object.
function installEarlyCaptureHook() {
  if (global.__saffulAntiDeleteEarlyCaptureInstalled) return;
  const previousLoad = Module._load;
  Module._load = function loadSaffulProtectionBaileys(request, parent, isMain) {
    ...
    const createProtectedSocket = (options = {}) => {
      const socket = previousCreateSocket(options);
      if (socket.__saffulAntiDeleteEarlyCaptureAttached) return socket;
      socket.__saffulAntiDeleteEarlyCaptureAttached = true;
      socket.ev.on('messages.upsert', ({ messages } = {}) => {
        for (const message of messages || []) cacheEarlyRawMessage(message);
      });
      // lib/smd.js can replace console.log with a no-op according to its log
      // configuration. This is a startup verification line, so bypass that
      // legacy setting and make the hook's presence observable on a host.
      process.stdout.write('[anti-delete] pre-core raw media capture attached\n');
      return socket;
    };
